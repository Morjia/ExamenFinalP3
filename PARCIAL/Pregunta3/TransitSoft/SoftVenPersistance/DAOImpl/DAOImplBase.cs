...
namespace SoftVen.DAOImpl
{
    public abstract class DAOImplBase
    {
        ...
        protected object EjecutarProcedimientoConParametroOut(string sql, Action<DbCommand> incluirParametrosIN, System.Data.DbType tipoSQL, string nombreParametroOut)
        {
            ...
            try
            {
                ...
                DbParameter parametroOut = this.comando.CreateParameter();
                parametroOut.ParameterName = nombreParametroOut;
                parametroOut.DbType = tipoSQL;
                parametroOut.Direction = System.Data.ParameterDirection.Output;
                this.comando.Parameters.Add(parametroOut);

                incluirParametrosIN?.Invoke(this.comando);

                this.comando.ExecuteNonQuery();
                resultado = parametroOut.Value;
            }
            catch (Exception ex)
            {
                Console.WriteLine("Error al ejecutar procedimiento con OUT - " + ex.Message);
            }
            finally
            {
                this.CerrarConexion();
