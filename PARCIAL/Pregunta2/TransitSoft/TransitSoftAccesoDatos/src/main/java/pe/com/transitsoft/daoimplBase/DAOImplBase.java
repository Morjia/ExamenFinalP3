...
public abstract class DAOImplBase {
    ...
    protected Object ejecutarProcedimientoConParametroOut(String sql,Consumer<CallableStatement> incluirParametrosIN,int tipoSQL,int indiceParametroOut) {
        Object resultado = null;
        try {
            this.abrirConexion();
            this.colocarSQLenStatement(sql);

            this.statement.registerOutParameter(indiceParametroOut, tipoSQL);

            if (incluirParametrosIN != null) {
                incluirParametrosIN.accept(this.statement);
            }

            this.statement.execute();
            resultado = this.statement.getObject(indiceParametroOut);

        } catch (SQLException ex) {
            System.err.println("Error al ejecutar procedimiento con OUT - " + ex);
        } finally {
            try {
                this.cerrarConexion();
            } catch (SQLException ex) {
