...
import pe.com.transitsoft.daoimpl.util.Tipo_Operacion;
...
public abstract class DAOImplBase {
    ...
    //Manejo de conexion y transacciones
    ...
    public void cerrarConexionSegura() {
        try {
            this.cerrarConexion();
...
//    CRUDS
    ...
    protected String generarSQLParaObtenerPorCorreo() {
        // 1) Construir lista de columnas: "col1, col2, col3"
        String sqlColumnas = "";
        for (Columna columna : this.listaColumnas) {
            if (!sqlColumnas.isBlank()) {
                sqlColumnas = sqlColumnas.concat(", ");
            }
            sqlColumnas = sqlColumnas.concat(columna.getNombre());
        }

        // 2) Montar la sentencia completa
        String sql = "SELECT "
                   + sqlColumnas
                   + " FROM "
                   + this.nombre_tabla
